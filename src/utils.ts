...
class SpotifyService {
    ...
    private async fetchCurrentSong(): Promise<SpotifyApiResponse | undefined> {
        const song = await this.fetchSpotifyApi("/me/player/currently-playing", { method: "GET" })
        if (!song) {
            await this.updateAccessToken()
            return await this.fetchSpotifyApi("/me/player/currently-playing", { method: "GET" })
        }
        return song
    }
    ...
    async playPause(): Promise<void> {
        let song = await this.fetchSpotifyApi("/me/player/currently-playing", { method: "GET" })
        if (!song || !song.is_playing) {
            await this.updateAccessToken()
            song = await this.fetchSpotifyApi("/me/player/currently-playing", { method: "GET" })
        }
        if (!song) {
            return
        }
